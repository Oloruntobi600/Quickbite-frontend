...
const Cart = () => {
  ...
  const { cart, auth } = useSelector(store => store.cart);  // Access cart and auth data from Redux store
  const addresses = useSelector(store => store.addresses) || [];  // Ensure addresses is an array
  const dispatch = useDispatch();
  ...
  const handleOpenAddressModel = () => {
    setOpen(true);  // Open the modal when this function is called
  };

  const handleClose = () => {
    setOpen(false);  // Close the modal when this function is called
  };

  const handleSubmit = (values) => {
    const addressData = selectedAddress || values;  // Use selected address or form values
    const data = {
      jwt: localStorage.getItem("jwt"),
      order: {
        restaurantId: cart.item[0].food?.restaurant.id,
